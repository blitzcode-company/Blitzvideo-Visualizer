...
export class VerStreamComponent implements OnInit, OnDestroy,  AfterViewInit, AfterViewChecked{
  ...
  mobileChatOpen: boolean = false;
  @ViewChild('mobileChat') mobileChat!: ElementRef;
  ...
  this.route.params.subscribe(params => {
   ...
   this.isCinemaMode = this.cinemaModeService.getCinemaModeValue();
    ...
    setTimeout(() => {
      ...
      console.log(`[COMPONENT] Iniciando escucha para stream ${this.streamId}`);
      ...
      this.streamEventsSub = this.chatService.getStreamEvents()
          .subscribe(event => {
            ...
            if (event.type === 'viewer_count' || event.viewer_count !== undefined || event.count !== undefined) {
              const viewerCount = event.count ?? event.viewer_count ?? 0;
              this.viewersCount = viewerCount;
              this.animarViewers(viewerCount);
  ...
  this.echoSuscrito = false; 
    ...
    if (this.streamId && (window as any).Echo) {
      try {
        ...
        console.log(`Desconectado del canal stream.${this.streamId}`);
      } catch (e) {
        console.warn('Error al desconectar del canal:', e);
      }
    }
...
checkDescriptionHeight(): void {
  ...
  detectMobile() {
    ...
    if (!this.isMobile && this.mobileChatOpen) {
